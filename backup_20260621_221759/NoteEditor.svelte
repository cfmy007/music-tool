<script lang="ts">
  ...
  let viewMode = $state<'score' | 'note'>(selectedNote.viewMode || 'score');
  ...
  async function handleViewModeChange(mode: 'score' | 'note') {
    viewMode = mode;
    selectedNote.viewMode = mode;
    try {
      await db.updateNoteMode(selectedNote.id, mode);
      // 切换到笔记模式时加载图层
      if (mode === 'note') {
        await loadLayers();
      }
  ...
  // 计算当前选中图层的可见性
  let currentLayerVisible = $derived.by(() => {
    ...
    {:else}
      <div class="viewer-wrapper">
        <ScoreViewer
          ...
          noteId={selectedNote.id}
          allPages={scorePages}
          layerVisible={currentLayerVisible}
          onNextPage={handleNextPage}
          onPrevPage={handlePrevPage}
